...
const Table = React.forwardRef<HTMLTableElement, TableProps>(
    ({ className, wrapperClassName, ...props }, ref) => (
        <div className={cn('relative w-full overflow-auto rounded-lg border border-slate-200 dark:border-slate-700', wrapperClassName)}>
            <table
                ref={ref}
...
>(({ className, ...props }, ref) => (
    <tfoot
        ref={ref}
        className={cn(
            'border-t bg-slate-50 font-medium dark:bg-slate-800/50',
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <th
        ref={ref}
        className={cn(
            'h-12 px-4 text-left align-middle font-semibold text-slate-600 dark:text-slate-300',
